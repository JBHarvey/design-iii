...
double checkCameraPose(const std::vector<cv::Point3f> &modelPts, const std::vector<cv::Point2f> &imagePts,
                       ...
                       const cv::Mat &distCoeffs, const cv::Mat &rvec, const cv::Mat &tvec)
{
    ...
    for(size_t i = 0; i < projectedPts.size(); i++) {
        rms += (projectedPts[i].x - imagePts[i].x) * (projectedPts[i].x - imagePts[i].x) + (projectedPts[i].y - imagePts[i].y)
               * (projectedPts[i].y - imagePts[i].y);
    }
...
cv::Point3f transformPoint(const cv::Point3f &pt, const cv::Mat &rvec, const cv::Mat &tvec)
{
    ...
    cv::Mat transformationMatrix = (cv::Mat_<double>(4, 4) << rotationMatrix.at<double>(0, 0), rotationMatrix.at<double>(0,
                                    1), rotationMatrix.at<double>(0, 2), tvec.at<double>(0),
    ...
    cv::Point3f transformedPt(transformedPtMat.at<double>(0), transformedPtMat.at<double>(1),
                              transformedPtMat.at<double>(2));
...
cv::Point3f transformPointInverse(const cv::Point3f &pt, const cv::Mat &rvec, const cv::Mat &tvec)
{
    ...
    cv::Mat transformationMatrix = (cv::Mat_<double>(4, 4) << rotationMatrix.at<double>(0, 0), rotationMatrix.at<double>(0,
                                    1), rotationMatrix.at<double>(0, 2), translation.at<double>(0),
...
void computePlaneEquation(const cv::Point3f &p0, const cv::Point3f &p1, const cv::Point3f &p2, float &a, float &b,
                          float &c, float &d)
{
    // Vector p0_p1
    cv::Point3f p0_p1;
...
cv::Point3f compute3DOnPlaneFrom2D(const cv::Point2f &imagePt, const cv::Mat &cameraMatrix, const float a,
...
void onMouseShowCoordinates(int evt, int x, int y, int flags, void* param)
{
    if(evt == CV_EVENT_LBUTTONDOWN) {
        cv::Point3f pt = transformPointInverse(compute3DOnPlaneFrom2D(cv::Point2f((float)x, (float)y), cameraMatrix, a, b, c,
                                               d), rvec, tvec);
        std::cout << "Point 3D coords: x=" << pt.x << ", y=" << pt.y << ", z=" << pt.z << std::endl;
    }
}

void onMouseGetPoints(int evt, int x, int y, int flags, void* param)
{
    if(evt == CV_EVENT_LBUTTONDOWN) {
...
int main()
{
    ...
    undistort(img, undistorted_img, cameraMatrix, distCoeffs, getOptimalNewCameraMatrix(cameraMatrix, distCoeffs,
              cv::Size(1600, 1200), 1.0, cv::Size(1600, 1200), 0, false));
    ...
    cv::setMouseCallback("Select points", onMouseGetPoints, (void*)&points);

    while(1) {
        cv::imshow("Select points", undistorted_img);
    ...
    for(size_t i = 0; i < pointbuf.size(); i++) {
        ...
        rms_3D += (modelPts[i].x - ptObjectFrame.x) * (modelPts[i].x - ptObjectFrame.x) + (modelPts[i].y - ptObjectFrame.y) *
                  (modelPts[i].y - ptObjectFrame.y) +
    ...
    cv::namedWindow("Explore projection");
    cv::setMouseCallback("Explore projection", onMouseShowCoordinates);
    cv::imshow("Explore projection", undistorted_img);
    cv::waitKey();
