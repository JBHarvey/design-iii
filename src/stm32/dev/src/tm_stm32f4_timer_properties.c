...
TM_TIMER_PROPERTIES_Result_t TM_TIMER_PROPERTIES_GetTimerProperties(
		TIM_TypeDef* TIMx, TM_TIMER_PROPERTIES_t* Timer_Data) {
	...
	TIMx == TIM1 || TIMx == TIM8 || TIMx == TIM9 || TIMx == TIM10
			|| TIMx == TIM11) {
	...
	TIMx == TIM3 || TIMx == TIM4 || TIMx == TIM6 || TIMx == TIM7
...
TM_TIMER_PROPERTIES_Result_t TM_TIMER_PROPERTIES_GenerateDataForWorkingFrequency(
		TM_TIMER_PROPERTIES_t* Timer_Data, double frequency) {
	...
	do {
		/* Get clock */
		Timer_Data->Period = (Timer_Data->TimerFrequency
				/ (Timer_Data->Prescaler + 1));
		/* Get period */
		...
		Timer_Data->Prescaler++;
			...
			&& Timer_Data->Prescaler <= (Timer_Data->MaxPrescaler + 1));
...
TM_TIMER_PROPERTIES_Result_t TM_TIMER_PROPERTIES_EnableClock(
		TIM_TypeDef* TIMx) {
	if (TIMx == TIM1) {
...
TM_TIMER_PROPERTIES_Result_t TM_TIMER_PROPERTIES_DisableClock(
		TIM_TypeDef* TIMx) {
	if (TIMx == TIM1) {
